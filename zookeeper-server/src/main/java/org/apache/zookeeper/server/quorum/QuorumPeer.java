...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public static class QuorumServer {
        ...
        private static final String wrongFormat = " does not have the form server_config or server_config;client_config" + " where server_config is host:port:port or " + "host" +
                ":port:port:type and client_config is port or host:port";
        ...
        private boolean checkAddressesEqual(InetSocketAddress addr1, InetSocketAddress addr2) {
            if ((addr1 == null && addr2 != null) || (addr1 != null && addr2 == null) || (addr1 != null && addr2 != null && !addr1.equals(addr2))) return false;
            return true;
        }
    ...
    public QuorumPeer(Map<Long,QuorumServer> quorumPeers, File dataDir, File dataLogDir, int electionType, long myid, int tickTime, int initLimit, int syncLimit,
    ...
    public QuorumPeer(Map<Long,QuorumServer> quorumPeers, File dataDir, File dataLogDir, int electionType, long myid, int tickTime, int initLimit, int syncLimit,
                      boolean quorumListenOnAllIPs, ServerCnxnFactory cnxnFactory, QuorumVerifier quorumConfig) throws IOException {
        this();
    ...
    public synchronized void start() {
        ...
        loadDataBase();
        // 这里会调用NIOServerCnxnFactory的start()方法
    ...
    public QuorumPeer(Map<Long,QuorumServer> quorumPeers, File snapDir, File logDir, int clientPort, int electionAlg, long myid, int tickTime, int initLimit, int syncLimit) throws IOException {
        this(quorumPeers, snapDir, logDir, electionAlg, myid, tickTime, initLimit, syncLimit, false, ServerCnxnFactory.createFactory(getClientAddress(quorumPeers, myid,
                clientPort), -1), new QuorumMaj(quorumPeers));
    }
     ...
     * It defaults to FileLogProvider persistence provider.
     */
    public QuorumPeer(Map<Long,QuorumServer> quorumPeers, File snapDir, File logDir, int clientPort, int electionAlg, long myid, int tickTime, int initLimit, int syncLimit,
                      QuorumVerifier quorumConfig) throws IOException {
        this(quorumPeers, snapDir, logDir, electionAlg, myid, tickTime, initLimit, syncLimit, false, ServerCnxnFactory.createFactory(getClientAddress(quorumPeers, myid,
                clientPort), -1), quorumConfig);
    }
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm) {
        ...
        switch (electionAlgorithm) {
            ...
            case 2:
                ...
                break;
            case 3:
                // QuorumCnxManager 负责发起网络请求（将投票发送出去或者接受别的节点发送过来的投票）
                // QuorumCnxManager 有三个内部类Listener线程、RecvWoker线程、SendWorker线程
                ...
                // Listener线程负责创建ServerSocket，用来接收投票信息
                // 当收到投票后，按以下流程处理网络数据
                ...
                // FastLeaderElection.start()内部启动了WorkerReceiver线程，会循环从recvQueue队列中拉数据，表现如下
                // manager.pollRecvQueue(3000, TimeUnit.MILLISECONDS);
                // 拉到消息后，然后把消息封装成Notification放到FastLeaderElection中的recvQueue<Notification>
                QuorumCnxManager.Listener listener = qcm.listener;
                if (listener != null) {
                    listener.start();
                    // FastLeaderElection是真正选举的地方
                    // 注意在这个构造方法里new了一个内部类new Messenger(manager)对象
                    // 同时Messenger又有两个内部类 WorkerReceiver 和 WorkerSender （他们都继承了Thread）
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                switch (getPeerState()) {
                    // 刚启动时候的状态就是LOOKING，表示是在寻找leader的过程中
                    // 如果此时还没有leader，此时就要发起投票给其他节点，尝试去选举leader
                    // 如果说已经有leader了，此时就让自己作为follower
                    case LOOKING:
                        LOG.info("LOOKING");
                        ...
                        if (Boolean.getBoolean("readonlymode.enabled")) {
                            ...
                            try {
                                ...
                                // lookForLeader方法内部是个while(true)循环，直到leader选举结束
                                setCurrentVote(makeLEStrategy().lookForLeader());
                            } catch (Exception e) {
                                LOG.warn("Unexpected exception", e);
                                setPeerState(ServerState.LOOKING);
    ...
    public QuorumVerifier setQuorumVerifier(QuorumVerifier qv, boolean writeToDisk) {
        synchronized (QV_LOCK) {
            ...
            quorumVerifier = qv;
            if (lastSeenQuorumVerifier == null || (qv.getVersion() > lastSeenQuorumVerifier.getVersion())) lastSeenQuorumVerifier = qv;
    ...
    public QuorumCnxManager createCnxnManager() {
        return new QuorumCnxManager(this, this.getId(), this.getView(), this.authServer, this.authLearner, this.tickTime * this.syncLimit, this.getQuorumListenOnAllIPs(),
                this.quorumCnxnThreadsSize, this.isQuorumSaslAuthEnabled());
    }
